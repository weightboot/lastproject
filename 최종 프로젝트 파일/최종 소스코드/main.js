...
const g_url = "http://172.20.221.166:42888";//시연 대비용
...
wfsSource2 = new VectorSource
  ...
  // let p_grade = feature.get('p_grade');
  // let p_arate1 = feature.get('p_arate');
  // if (p_arate1==null) {
  //   color = 'rgba(0, 255, 0, 0.6)'; // 녹색
  // }
  
  // else if (grade02 && grade === '나') {
  //   color = 'rgba(255, 255, 0, 0.6)'; // 노란색
  // } else if (grade03 && grade === '다') {
  //   color = 'rgba(255, 165, 0, 0.6)'; // 주황색
  // } else if (grade04 && grade === '라') {
  //   color = 'rgba(255, 0, 0, 0.6)'; // 빨간색
  // }else if (grade05 && grade === '마') {
  //   color = 'rgba(0, 0, 255, 0.6)'; // 파랑
  // }
  function getFeatureStyle1(feature) {
    let p_grade = feature.get('p_grade');
    let p_arate1 = feature.get('p_arate');
    let color = 'rgba(145, 145, 145, 0.6)'; // 기본
  
    if (p_arate1 !=null && p_grade=='가' ) {
      color = 'rgba(0, 255, 0, 0.6)'; // 녹색
    } else if (p_arate1 !=null && p_grade== '나') {
      color = 'rgba(255, 255, 0, 0.6)'; // 노란색
    } else if (p_arate1 !=null && p_grade== '다') {
      color = 'rgba(255, 165, 0, 0.6)'; // 주황색
    } else if (p_arate1 !=null && p_grade=='라') {
      color = 'rgba(255, 0, 0, 0.6)'; // 빨간색
    }else if (p_arate1 !=null && p_grade=='마') {
      color = 'rgba(0, 0, 255, 0.6)'; // 파랑
    }
...
wfsLayer2 = new VectorLayer
  (
    {
      source: wfsSource2,
      style: function (feature) { return getFeatureStyle1(feature); }
    }
  );
...
function getCQLfilter() {
  ...
  const grade01 = document.getElementById("grade01");
  const grade02 = document.getElementById("grade02");
  const grade03 = document.getElementById("grade03");
  const grade04 = document.getElementById("grade04");
  const grade05 = document.getElementById("grade05");
  ...
  if ((true == grade01.checked) || (true == grade02.checked) || (true == grade03.checked) || (true == grade04.checked) || (true == grade05.checked)) {
    if (0 < sCQL.length)
      sCQL += " and "
    sCQL += "(";
  }

  if (true == grade01.checked) {
    sCQL += "grade = '가'"
  }

  if (true == grade02.checked) {
    if (sCQL.charAt(sCQL.length - 1) != '(')
      sCQL += " or "
    sCQL += "grade='나'"
  }

  if (true == grade03.checked) {
    if (sCQL.charAt(sCQL.length - 1) != '(')
      sCQL += " or "
    sCQL += "grade='다'"
  }

  if (true == grade04.checked) {
    if (sCQL.charAt(sCQL.length - 1) != '(')
      sCQL += " or "
    sCQL += "grade='라'"
  }

  if (true == grade05.checked) {
    if (sCQL.charAt(sCQL.length - 1) != '(')
      sCQL += " or "
    sCQL += "grade='마'"
  }
...
function getFeatureStyle(feature) {
  ...
  const grade01 = document.getElementById("grade01").checked;
  const grade02 = document.getElementById("grade02").checked;
  const grade03 = document.getElementById("grade03").checked;
  const grade04 = document.getElementById("grade04").checked;
  const grade05 = document.getElementById("grade05").checked;
...
const map = new Map({
  target: 'map',
  layers: [
    osmLayer, vworldSatelliteLayer, wfsLayer, vector, wfsLayer2
  ],
  view: new View({
...
document.getElementById("landyview_top").style.display = 'none';
document.getElementById("landyview_bottom").style.display = 'none';
document.getElementById("landyview_id").style.backgroundColor = 'transparent';
...
map.on('click', (e) => {
  ...
  map.forEachFeatureAtPixel(e.pixel, (feature, layer) => {
    ...
    let arate = feature.get('arate');
    let erate = feature.get('erate');
    let rgarate = feature.get('rgarate');
    let rgdist = feature.get('rgdist');
    let fmdist = feature.get('fmdist');
    let seadist = feature.get('seadist');
    let high = feature.get('high');
    let slant = feature.get('slant');
    let cvdist = feature.get('cvdist');
    let price = feature.get('price');
    let rdist = feature.get('rdist');
    let ddist = feature.get('ddist');

    let dv_sumscore = feature.get('dv_sumscore');
    let cs_sumscore = feature.get('cs_sumscore');
    let t_score = feature.get('t_score');
    let grade = feature.get('grade');


    let p_arate = feature.get('p_arate');
    let p_erate = feature.get('p_erate');
    let p_rgarate = feature.get('p_rgarate');
    let p_rgdist = feature.get('p_rgdist');
    let p_fmdist = feature.get('p_fmdist');
    let p_seadist = feature.get('p_seadist');
    let p_high = feature.get('p_high');
    let p_slant = feature.get('p_slant');
    let p_cvdist = feature.get('p_cvdist');
    let p_price = feature.get('p_price');
    let p_rdist = feature.get('p_rdist');
    let p_ddist = feature.get('p_ddist');

    let p_dv_sumscore = feature.get('p_dv_sumscore');
    let p_cs_sumscore = feature.get('p_cs_sumscore');
    let p_t_score = feature.get('p_t_score');
    let p_grade = feature.get('p_grade');




    if (address == null &&p_high==null) {
    ...
    } else if(address == null && p_high!=null){
      document.getElementById("insert_form").style.display = 'block';
      document.getElementById("myForm").style.display = 'block';

      document.getElementById("landyview_top").style.display = 'none';
      document.getElementById("landyview_bottom").style.display = 'block';
      document.getElementById("landyview_id").style.backgroundColor = '#0CEBC9 ';

    } else {
      document.getElementById("insert_form").style.display = 'block';
      document.getElementById("myForm").style.display = 'block';

      document.getElementById("landyview_top").style.display = 'block';
      document.getElementById("landyview_bottom").style.display = 'none';
      document.getElementById("landyview_id").style.backgroundColor = '#0CEBC9 ';
    };
    ...
    document.getElementById("slant1_display").innerHTML = slant;
    document.getElementById("high_display").innerHTML = high;
    document.getElementById("ddist_display").innerHTML = ddist;
    document.getElementById("CVdist_display").innerHTML = cvdist;
    document.getElementById("price_display").innerHTML = price;
    document.getElementById("Rdist_display").innerHTML = rdist;
    document.getElementById("Arate_display").innerHTML = arate;
    document.getElementById("Erate_display").innerHTML = erate;
    document.getElementById("RGArate_display").innerHTML = rgarate;
    document.getElementById("RGdist_display").innerHTML = rgdist;
    document.getElementById("FMdist_display").innerHTML = fmdist;
    document.getElementById("Seadist_display").innerHTML = seadist;
    document.getElementById("dv_sumsocre_display").innerHTML =dv_sumscore;
    document.getElementById("cs_sumsocre_display").innerHTML =cs_sumscore;
    document.getElementById("t_socre_display").innerHTML =t_score;
    document.getElementById("grade_display").innerHTML =grade;


    document.getElementById("p_slant1_display").innerHTML = p_slant;
    document.getElementById("p_high_display").innerHTML = p_high;
    document.getElementById("p_ddist_display").innerHTML = p_ddist;
    document.getElementById("p_CVdist_display").innerHTML = p_cvdist;
    document.getElementById("p_price_display").innerHTML = p_price;
    document.getElementById("p_Rdist_display").innerHTML = p_rdist;
    document.getElementById("p_Arate_display").innerHTML = p_arate;
    document.getElementById("p_Erate_display").innerHTML = p_erate;
    document.getElementById("p_RGArate_display").innerHTML = p_rgarate;
    document.getElementById("p_RGdist_display").innerHTML = p_rgdist;
    document.getElementById("p_FMdist_display").innerHTML = p_fmdist;
    document.getElementById("p_Seadist_display").innerHTML = p_seadist;
    document.getElementById("p_dv_sumsocre_display").innerHTML =p_dv_sumscore;
    document.getElementById("p_cs_sumsocre_display").innerHTML =p_cs_sumscore;
    document.getElementById("p_t_socre_display").innerHTML =p_t_score;
    document.getElementById("p_grade_display").innerHTML =p_grade;
...
document.getElementById('grade01').onchange = () => {
  console.log('exclude04 clicked');
  WFSfiltermaker();
}

document.getElementById('grade02').onchange = () => {
  console.log('exclude04 clicked');
  WFSfiltermaker();
}

document.getElementById('grade03').onchange = () => {
  console.log('exclude04 clicked');
  WFSfiltermaker();
}

document.getElementById('grade04').onchange = () => {
  console.log('exclude04 clicked');
  WFSfiltermaker();
}

document.getElementById('grade05').onchange = () => {
  console.log('exclude04 clicked');
  WFSfiltermaker();
}
